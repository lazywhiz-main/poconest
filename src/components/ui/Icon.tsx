...
const icons: Record<IconName, (props: { size: number; color: string }) => JSX.Element> = {
  ...
  'ai-summary': ({ size, color }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M12 2L2 7v10c0 5.55 3.84 9.74 9 11 5.16-1.26 9-5.45 9-11V7l-10-5z"/>
      <path d="M8 12h8M8 8h8M8 16h5"/>
    </svg>
  ),
  'card-extract': ({ size, color }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
const Icon: React.FC<IconProps> = ({ name, size = 20, color = 'currentColor', style }) => {
  const SvgIcon = icons[name];
  if (!SvgIcon) return null;
  return <span style={{ display: 'inline-flex', alignItems: 'center', ...style }}><SvgIcon size={size} color={color} /></span>;
};
