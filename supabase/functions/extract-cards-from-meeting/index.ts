...
- tags: 発話の引用部分（> 発言者: ...）からのみ抽出するタグ配列
  * 形容詞・動詞：人の行動や感情を表現するもの（例："驚いた", "混乱している", "決断した", "躊躇した", "共感する"）3-7個程度
  * 名詞・概念：重要なキーワード、専門用語、具体的な対象物（例："記憶", "文化", "言語", "関係性", "価値観"）3-7個程度
  * 一般的形容詞（"重要な", "新しい", "大きな"など）や抽象的すぎる名詞は避ける
  * 全体で1カードあたり6-14個程度（形容詞・動詞と名詞・概念のバランスを取る）
